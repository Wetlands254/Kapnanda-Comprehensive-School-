-- KAPNANDA PRIMARY AND JUNIOR SCHOOL MANAGEMENT SYSTEM - V2
...
-- Shared cloud state used by the Kapnanda GitHub Pages frontend.
...
drop policy if exists "school_state_shared_select_auth" on public.school_state_shared;
drop policy if exists "school_state_shared_insert_auth" on public.school_state_shared;
drop policy if exists "school_state_shared_update_auth" on public.school_state_shared;
create policy "school_state_shared_select_auth" on public.school_state_shared for select to authenticated using (true);
create policy "school_state_shared_insert_auth" on public.school_state_shared for insert to authenticated with check (true);
create policy "school_state_shared_update_auth" on public.school_state_shared for update to authenticated using (true) with check (true);

-- Default school settings. This inserts no learners, teachers, marks, or fees.
insert into public.school_settings (school_name, motto, current_term, academic_year)
select 'Kapnanda Primary and Junior School', 'Learn • Grow • Excel', 'Term 3', 2026
