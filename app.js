...
function Rankings(){
  ...
  document.getElementById("content").innerHTML=`<div class="card ranking-page">
    ...
    <div class="toolbar no-print"><button onclick="renderOverallRanking()">Show Ranking</button><button class="light" onclick="printRanking()">🖨️ Print A4 Class Ranking</button></div>
...
function renderOverallRanking(){
  ...
  document.getElementById("rankout").innerHTML=`<div id="rankingPrintTarget" class="report ranking-report">
...
// Keep this deployment connected to the intended Kapnanda Supabase project.
try{
  const saved=JSON.parse(localStorage.getItem(CLOUD_KEY)||"null");
  if(!saved || saved.url!==DEFAULT_SUPABASE_CONFIG.url || saved.anonKey!==DEFAULT_SUPABASE_CONFIG.anonKey){
    localStorage.setItem(CLOUD_KEY,JSON.stringify(DEFAULT_SUPABASE_CONFIG));
  }
}catch(e){
  localStorage.setItem(CLOUD_KEY,JSON.stringify(DEFAULT_SUPABASE_CONFIG));
}
if(cloudConfigured()){
  ...
  if(mode)mode.textContent="Supabase mode — sign in with your Supabase Auth email.";
...
function printRanking(){
  const target=document.getElementById("rankingPrintTarget")||document.getElementById("rankout");
  if(!target||!target.querySelector(".ranking-report")){toast("Generate the class ranking first");return}
  const style=document.createElement("style");
  style.id="rankingLandscapePrintStyle";
  style.textContent=`@media print{
    @page{size:A4 landscape;margin:7mm}
    body{background:#fff!important;-webkit-print-color-adjust:exact;print-color-adjust:exact}
    .ranking-page>h2,.ranking-page>.pagehero,.ranking-page>.formgrid,.ranking-page>.toolbar{display:none!important}
    .ranking-page{display:block!important;border:0!important;box-shadow:none!important;padding:0!important}
    .ranking-page>#rankout{display:block!important;margin:0!important}
    .main{margin:0!important;width:100%!important;padding:0!important}
    .ranking-report{display:block!important;width:100%!important;max-width:none!important;border:0!important;padding:0!important;margin:0!important}
    .ranking-report .schoolhead{margin-bottom:6px!important}
    .ranking-report .schoolhead h1{font-size:18px!important}
    .ranking-report .schoolhead p{font-size:9px!important;margin:2px 0!important}
    .ranking-report h2{font-size:11px!important;margin:6px 0 3px!important}
    .ranking-report th,.ranking-report td{font-size:7px!important;padding:3px!important}
    .ranking-report .class-ranking-table th,.ranking-report .class-ranking-table td{font-size:6.5px!important;padding:2.5px!important}
    .ranking-report .subject-performance{line-height:1.2!important}
    .ranking-footer{font-size:6.5px!important}
  }`;
  document.head.appendChild(style);
  document.body.classList.add("printing-ranking");
  window.print();
  setTimeout(()=>{document.body.classList.remove("printing-ranking");style.remove()},800);
}
